...
def put_ship_on_board(ship, player, length):
    try:
        ...
        if start_row == end_row:  # Horizontal placement (rows are the same, Ex: A0-A4 -> 0 == 0 (since "A" is converted to 0 above))
        ...
        elif start_col == end_col:  # Vertical placement (columns are the same, Ex: A5-E5 -> 5 == 5)
...
def attempt_move(current_player, input_move): # ex for input_move A5
    ...
    try:
        ...
        if opp_board[move_row][move_col] != "~":  # It's a hit
            ...
            send_client_msg(client_sockets[current_player], "\nHit! (Your Turn Again)\n")
            send_client_msg(client_sockets[get_other_player(current_player)], "You got hit! (" + input_move + ")\n" + print_client_boards(get_other_player(current_player)))
            ...
            # Inform the client the result of the move
            send_client_msg(client_sockets[current_player], "\nMiss! (Turn Change)\n" + print_client_boards(current_player))
...
# Note: This could also be changed to use the ship_sunk array, but both will return the same result.
...
# Prints both the move and placement boards for a specified player
def print_client_boards(player):
    return "\nYour Moves:\n" + print_board(move_boards[player]) + "\nYour Board:\n" + print_board(client_boards[player]) + "\n"
...
def handle_client(server_socket, client_socket, client_number):
    ...
    try:
        send_client_msg(client_socket, "You are player " + str(client_number + 1))
        ...
        send_client_msg(client_socket, "\nYour ship placements are complete. Waiting for the game to start...\n\nYour Board:\n" + print_board(client_boards[client_number]))
        ...
        while True:
            ...
            send_client_msg(client_socket, print_client_boards(client_number) + "Input Your Move\n")

            move = get_client_msg(client_socket)
            
            # Not enough players
            if client_count < 2:
                send_client_msg(client_socket, "\nOther client has not yet connected")
...
def start_server():
    global client_sockets, client_count
    ...
    try:
        ...
        print("Could not start server (possibly due to the server already running).")
        return
    ...
    while True:
        ...
        # This is only for the listening logic, which fails once the server socket is closed
        # Any error with the server socket itself should be caught in the try-except above this
        except Exception:
            break
